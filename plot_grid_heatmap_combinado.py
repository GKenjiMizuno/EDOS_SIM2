...
# Heatmap estilo gráfico 21 (ataque, wu_calibration, 1 amostra/célula),
# mas para COMBINADO (ataque + 40 agregado de tráfego normal padrão) COM
# REPETIÇÃO -- "fecha" o cenário combinado, par direto do gráfico 28
# (isolado com repetição). Completa o trio de grids: 17 = só tráfego
# normal, 30 (este) = combinado, 28 = só ataque isolado.
#
# Fonte: experiment_results/desempate_combinado_isolado/ (seção 62/66),
# condição "combinado", que já tem 5 REPETIÇÕES por célula nas 9
# combinações (rps=1/5/10 × WU=100k/200k/400k) -- melhor que a amostra
# única de experiment_results/wu_calibration/ que alimenta o gráfico 21
# original. Nenhum experimento novo -- dado já coletado na investigação
# da seção 62/66. Cada célula mostra média±desvio de CPU (não 1 amostra
# só), RPS agregado, taxa de timeout, e nº máximo de instâncias
...
for i, rps in enumerate(RPS_VALUES):
    for j, wu in enumerate(WU_VALUES):
        ...
        for rep in range(1, NUM_REPS + 1):
            suffix = f"combinado_rps{rps}_att{NUM_ATTACKERS}_WU{wu}_rep{rep}"
...
ax.set_title(
    "Grid de ataque COMBINADO (+ 40 agregado normal) -- CPU média por célula\n"
    "5 repetições/célula -- comparar com gráfico 28 (isolado) e 21 (amostra única)",
    fontsize=11,
)
fig.tight_layout()
out_path = os.path.join(OUT_DIR, "30_grid_heatmap_combinado.png")
